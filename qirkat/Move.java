...
class Move {
    ...
    /* To avoid creating Move objects that are not needed, we maintain
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            char move1col1 = move1.col1();
            char move1row1 = move1.row1();
            Move newMove1 = move(move1col1, move1row1);

            char move0col0 = move0.col0();
            char move0row0 = move0.row0();
            char move1col0 = move1.col0();
            char move1row0 = move1.row0();

            return move(move0col0, move0row0, move1col0,
                    move1row0, newMove1);
        }
    ...
    boolean isLeftMove() {
        if (this.isJump()) {
            return false;
        } else if (validSquare(this._col1, this._row1)
                && this._col1 == this._col0 - 1
                && this._row1 == this._row0) {
            return true;
        }
        return false;
    }
    ...
    char jumpedRow() {
        ...
        char[] cols = {'a', 'b', 'c', 'd', 'e'};
        if (isJump()) {
            if (this._row1 == this._row0) {
                return this._row1;
            }
            if (this._row1 > this._row0) {
                for (int i = 0; i < rows.length; i++) {
                    if (rows[i] == this._row1) {
                        return rows[i - 1];
                    }
                }
            }  else {
    ...
    char jumpedCol() {
        ...
        char[] cols = {'a', 'b', 'c', 'd', 'e'};
        if (isJump()) {
            if (this._col1 == this._col0) {
                return this._col1;
            }
            if (this._col1 > this._col0) {
                for (int i = 0; i < cols.length; i++) {
                    if (cols[i] == this._col1) {
                        return cols[i - 1];
                    }
                }
            } else {
     ...
     * @param out Formatter
     */
    private void toString(Move mov, Formatter out) {
        if (mov.jumpTail() != null) {
            out.format(Character.toString(mov.col0())
                    + Character.toString(mov.row0()) + "-");
            toString(mov.jumpTail(), out);
        } else {
            out.format(Character.toString(mov.col0())
                    + Character.toString(mov.row0())
                    + "-"
                    + Character.toString(mov.col1())
                    + Character.toString(mov.row1()));
        }
